void make_topviewhdr(const char* InputFile,
		     const char* ClassName = "AtlTopViewProxy") {
    //
    // Generate .h and .cxx file for given TopView ntuple file with
    ...
    TTree *t_collection = (TTree*)gDirectory->Get("CollectionTree");
    TTree *t_fullreco0 = (TTree*)gDirectory->Get("FullReco0");
    TTree *t_truth0 = (TTree*)gDirectory->Get("Truth0");
    t_fullreco0->SetTreeIndex(0);
    t_truth0->SetTreeIndex(0);
    t_collection->AddFriend(t_fullreco0);
    t_collection->AddFriend(t_truth0);

    // Make proxy
    t_collection->MakeProxy(ClassName, MacroFile.Data());
...
// !! cmd/make_topviewhdr with the help of the      !!
