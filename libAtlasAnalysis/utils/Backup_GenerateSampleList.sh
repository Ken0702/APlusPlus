...
for Line in $(cat $DatasetList | grep -v "^#" | grep -v "^$"); do
    ...
    if [ "${Campaign}" != mc12 ] ; then
        ...
        case ${SampleType} in
            kDATA*)
		for folder in $( find /rdsk/$MSAS*/atlas/SgTopD3PD/${Campaign}_${Version}/DATA -type d -name "user*.*${DSID}.*" ) ; do
                    if [ "${append}" -gt 0 ] ; then
			echo + + + + "${folder}"
                    else
		...
		;;
            kQCD_AntiMu*)
                for folder in $( find /rdsk/$MSAS*/atlas/SgTop_Fake/AntiMuon_${Version} -type d -name "user*.*.${DSID}.*" ) ; do
			...
			continue
                ...
                ;;
            kQCD_JetLep*)
                for folder in $( find /rdsk/$MSAS*/atlas/SgTop_Fake/JetLepton_${Version} -type d -name "user*${reco}*" ) ; do
                    if [ "${append}" -gt 0 ] ; then
                        echo + + + + "${folder}"
                    else
                        echo "${SampleShort}" "${SampleType}" + "${Color}" "${folder}"
		...
		if [ "x${Xsection}" = x ] && [ "${DSID}" != 957011 ] ; then
			...
			Xsection=0.1 # <--- 0.1 pb for sigma*BR
                    ...
                    for folder in $( find /rdsk/$MSAS*/atlas/SgTopD3PD/${Campaign}_${Version}/MC -type d -name "user*.mc*TeV.${DSID}.*" ) ; do
