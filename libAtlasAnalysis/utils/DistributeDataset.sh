...
echo "'$NFILES' datasets to be distributed on '$NDISKS'"
...
for disk in $(echo $3 | awk -F":" '{ for(i=1;i<=$NF;i++) print $i }'); do
	...
	if [ "$source_disk" = "$target_disk" ]; then
		...
		if [ $MSAS = $dest_machine ]; then
		    echo "mv $source_folder $target_folder"
		    mv $source_folder $target_folder
		    chmod g+w -R $target_folder
		else
		    echo "cp -r $source_folder $target_folder"
...
echo "Total number of files copied......: '$COPIED'"
echo "Total number of files to be copied: '$NFILES'"
