...
void TKinFitter::setParamMeas( Int_t index ) {
  // Declares a parameter as measured
    ...
    if ( index >= 0 && index < (Int_t)_paramMeasured.size() ) {
	resetStatus();
...
void TKinFitter::setParamMeas( TAbsFitParticle* particle, Int_t index ) {
  ...
  if ( parIndex >= 0 && parIndex < (Int_t)_paramMeasured.size() ) {
    resetStatus();
...
Int_t TKinFitter::getParameterIndex( TAbsFitParticle* particle, Int_t aPar ) {
  ...
  Int_t parIndex = aPar;
  for ( UInt_t index = 0; index < (UInt_t)pIndex; index++ ) {
    TAbsFitParticle* aParticle = _particles[index];
...
Int_t TKinFitter::GetEtaNuRoot( TLorentzVector P, TVector2 pNu, Double_t M, 
				Double_t &Root1, Double_t &Root2, Double_t &Minimum ) {
  
  // helper class for setting starting value of unmeasured eta in topologies with
  // one non-interacting particle
  // -> looking for Roots of the constraint equation (P+pNu)^2-M^2=0
  // -> if there are Roots then these are written in &Root1 and &Root2, NRoot==2
  // -> if not then the Minimum value of the constraint is written in &Minimum, NRoot==0
    
  Int_t NRoot=2; // Number of Roots
  Double_t Mp=P.M();
  Double_t h=0.5*(M*M-Mp*Mp)+P.X()*pNu.X()+P.Y()*pNu.Y();  
  Double_t Pz2=P.Z()*P.Z();
  Double_t E2=P.E()*P.E();
  Double_t EtMiss=pNu.Mod();
  Double_t a=h*P.Z()/(E2-Pz2);  
  Double_t D=a*a-(E2*EtMiss*EtMiss-h*h)/(E2-Pz2);// Wrong ? Denominator = E2+Pz2 ?
  
  if ( D<0 ) {
    NRoot=0;
    Minimum=TMath::ASinH(P.Z()/P.Pt());
  }
  else {
    Root1=a-sqrt(D);
    Root1=TMath::ASinH(Root1/EtMiss);
    Root2=a+sqrt(D);
    Root2=TMath::ASinH(Root2/EtMiss);
  }
...
Int_t TKinFitter::fit() {
  ...
  do {
    // Debugging
    //if (_nbIter<5) {
// 	  cout <<  endl << "====== Before Fitter Iteration" << _nbIter+1 << "======"  << endl;
...
// 	  }
    ...
    // Calculate correction for a, y, and lambda
    ...
    // testing modification of the fitting procedure
    // reason: problems with TFitConstraintMBW in Single Top t-channel analysis
    // there neutrino eta (parameter, called 'a' here, unmeasured) is problematic
    // therefore reduce step width for 'a' if F gets bigger. 
    // (S does not matter here since parameters 'a' do not appear in S)
    
//     if ( _nParA>0 ){ // only if there are unmeasured parameters
//       Int_t nIterMod=0;
//       while ( currF > prevF && nIterMod<3 ) {
// 	nIterMod++;
// 	_deltaA *= 0.;
// 	applyDeltaAY();
// 	currF=getF();
...
//       // debugging
//       cout << endl << "reduced step width for parameters" << endl;
      
//       if ( currF>prevF ) { 
// 	// reducing step width not helpful here
// 	// keep parameter value of last iteration,
// 	// that means deltaA=deltaA*
	
// 	// calculate delta(a*), = 0 in the first iteration
// 	// this part is copied from below and should be written as a subroutine
...
// 	// debugging
// 	cout << endl << "but kept the parameter value of the previous iteration" << endl;
...
//     if (_nbIter<5) {
//       cout << endl << endl << "========== Fitter Iteration" << _nbIter << "============" << endl << endl;
      
//       cout << "|DeltaS| = " << TMath::Abs(prevS-currS) << endl;
//       cout << "S        = " << currS << endl;
//       cout << "|F|      = " << TMath::Abs(currF) << endl;
      
//       if ( isConverged ) { cout << "Convergence: YES." << endl; }
//       else { cout << "Convergence: NO." << endl; }
//     }
...
//   cout <<  "Inverting V." <<endl;

  Double_t det = 0.;
  
  _Vinv.Invert(&det);
  if ( det == 0 ) _matrix_inv_failed = true;
...
Bool_t TKinFitter::calcA() {
  ...
  for (UInt_t indexConstr = 0; indexConstr < _constraints.size(); indexConstr++) {
    ...
    for (UInt_t indexParticle = 0; indexParticle < _particles.size(); indexParticle++) {
      ...
      for (UInt_t indexParam = 0; indexParam < (UInt_t)deriv.GetNcols(); indexParam++) {
...
Bool_t TKinFitter::applyDeltaAY() {
  // apply corrections to measured and unmeasured 
...
//   if ( isConverged ) cout << endl << "Fitter Converged!" << endl;
