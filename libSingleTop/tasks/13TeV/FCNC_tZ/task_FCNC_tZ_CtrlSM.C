...
//
// SM tZ trilepton ctrl region
//
// Author: Ana Peixoto <mailto: ana.paula.pereira.peixoto@cern.ch>
// Update: $Id$
// Copyright: 2015 (C) Ana Peixoto
//
void task_FCNC_tZ_CtrlSM() {

    // ========================
    // Configure top-level task
    // ========================
    AtlTopLevelAnalysis *main = new AtlTopLevelAnalysis("AtlSgTopFCNC_tZ_CtrlSM",
							"FCNC tZ, ctrl SM");
    ...
    main->SetHistDir(Form("/rdsk/datm5/atlas/%s/sgtop_fcnc_tZ_ctrl_sm", user));
    main->SetJobHomeDir(Form("/rdsk/datm5/atlas/%s/sgtop_fcnc_tZ_ctrl_sm", user));
    main->SetTempDir(Form("/datc/atlas/%s/sgtop_fcnc_tZ_ctrl_sm.tmp", user));
    ...
    main->SetOutputTree("physics", "SgTopD3PD FCNC tZ CR SM");
    ...
    main->SetSelector("AtlSgTopFCNC_tZ_CtrlSM");
