//
// Lpton fakes study for the SgTop 13TeV tZ FCNC analysis
//
// Signal-like region for ttbar bkg.
//
// Author: Oliver Maria Kind <mailto: kind@physik.hu-berlin.de
// Update: $Id$
// Copyright: 2017 (C) Oliver Maria Kind 
//
void task_FCNC_tZ_LepFakes_Ttbar() {

    // ========================
    // Configure top-level task
    // ========================
    AtlTopLevelAnalysis *main = new AtlTopLevelAnalysis("AtlSgTopFCNC_tZ_LepFakes_Ttbar",
							"FCNC tZ, lepton fakes, ttbar");
    ...
    main->SetHistDir(Form("/rdsk/datm5/atlas/%s/sgtop_fcnc_tZ_lepfakes_ttbar", user));
    main->SetJobHomeDir(Form("/rdsk/datm5/atlas/%s/sgtop_fcnc_tZ_lepfakes_ttbar", user));
    main->SetTempDir(Form("/datc/atlas/%s/sgtop_fcnc_tZ_lepfakes_ttbar.tmp", user));
    ...
    main->SetOutputTree("physics", "SgTopD3PD FCNC tZ lepton fakes ttbar");
    ...
    // // Read MC truth tree (caution: time-consuming)
    // main->SetParameter("fDoTruthTree", "kTRUE");
    
    // Set jet multiplicity (at least 1 jet)
    main->AddJetMultiplicity(AtlSelector::kTwoJet);

    // Set lepton channel (e or mu)
    main->AddLeptonChannel(AtlSelector::kLepton);
    
    // Analysis selector + Cuts
    main->SetSelector("AtlSgTopFCNC_tZ_Signal");
    ...
    main->SetCut("fNLeptons_min", "3");
    main->SetCut("fNLeptons_max", "3");
    ...
    main->SetToolParameter("BDecayGenTool", "fValidDSIDs", "\"410009\"");
    main->SetToolParameter("BDecayGenTool", "fBJetMode", "AtlBDecayGenTool::kLowPt");
    //main->SetToolParameter("BDecayGenTool", "fDoOverlapRemoval", "false");
