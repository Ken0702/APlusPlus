...
//
void task_FCNC_tZ_Ctrlttbar2L() {

    // ========================
    // Configure top-level task
    // ========================
    AtlTopLevelAnalysis *main = new AtlTopLevelAnalysis("AtlSgTopFCNC_tZ_Ctrlttbar2L",
							"FCNC tZ 2L, Ctrl Ttbar");
    ...
    main->SetCrossSectionFilePath("/cvmfs/atlas.cern.ch/repo/sw/ASG/AnalysisTop/2.4.29/TopDataPreparation/data/XSection-MC15-13TeV-fromSusyGrp.data");
    main->SetDSIDList("../SampleList/SgTopDSIDs_mc15_v15.txt");
    main->ReadSampleList("../SampleList");

    // Setup Directories
    char const* user = gSystem->Getenv("USER");
    main->SetHistDir(Form("/rdsk/datm5/atlas/%s/sgtop_fcnc_tZ_ctrl_ttbar2l", user));
    main->SetJobHomeDir(Form("/rdsk/datm5/atlas/%s/sgtop_fcnc_tZ_ctrl_ttbar2l", user));
    main->SetTempDir(Form("/datc/atlas/%s/sgtop_fcnc_tZ_ctrl_ttbar2l.tmp", user));
    ...
    main->SetOutputTree("physics", "SgTopD3PD FCNC tZ 2L CR Ttbar");
    ...
    // Set jet multiplicity (exactly 2 jets)
	...
	main->SetSelector("AtlSgTopFCNC_tZ_Ctrlttbar2L");
