...
//
// Z+jets trilepton ctrl region
//
// Author: Oliver Maria Kind <mailto: kind@physik.hu-berlin.de
// Update: $Id$
// Copyright: 2015 (C) Oliver Maria Kind 
//
void task_FCNC_tZ_CtrlZjets3L() {

    // ========================
    // Configure top-level task
    // ========================
    AtlTopLevelAnalysis *main = new AtlTopLevelAnalysis("AtlSgTopFCNC_tZ_CtrlZjets3L",
							"FCNC tZ, ctrl Z+jets+3L");
    ...
    main->SetHistDir(Form("/rdsk/datm5/atlas/%s/sgtop_fcnc_tZ_ctrl_zjets3l", user));
    main->SetJobHomeDir(Form("/rdsk/datm5/atlas/%s/sgtop_fcnc_tZ_ctrl_zjets3l", user));
    main->SetTempDir(Form("/datc/atlas/%s/sgtop_fcnc_tZ_ctrl_zjets3l.tmp", user));
    ...
    main->SetOutputTree("physics", "SgTopD3PD FCNC tZ CR Zjets3L");
	...
	main->SetSelector("AtlSgTopFCNC_tZ_CtrlZjets3L");
