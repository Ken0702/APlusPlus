...
//
// Signal region with a cut on M(top)
//
// Author: Oliver Maria Kind <mailto: kind@physik.hu-berlin.de
// Update: $Id$
// Copyright: 2015 (C) Oliver Maria Kind 
//
void task_FCNC_tZ_Signal_Offtop() {

    // ========================
    // Configure top-level task
    // ========================
    AtlTopLevelAnalysis *main = new AtlTopLevelAnalysis("AtlSgTopFCNC_tZ_Signal",
							"FCNC tZ 3L, Signal");
    ...
    main->SetHistDir(Form("/rdsk/datm5/atlas/%s/sgtop_fcnc_tZ_signal_offtop", user));
    main->SetJobHomeDir(Form("/rdsk/datm5/atlas/%s/sgtop_fcnc_tZ_signal_offtop", user));
    main->SetTempDir(Form("/datc/atlas/%s/sgtop_fcnc_tZ_signal_offtop.tmp", user));
    ...
    main->SetOutputTree("physics", "SgTopD3PD FCNC tZ SR 3L");
    ...
    main->SetCut("fMassTop_min", "153.34");
	main->SetCut("fMassTop_max", "193.34");
    main->SetCut("fVetoTop","kTRUE");
