...
//
// Signal-like region for Z+jets bkg.
//
// Author: Oliver Maria Kind <mailto: kind@physik.hu-berlin.de
// Update: $Id$
// Copyright: 2017 (C) Oliver Maria Kind 
//
void task_FCNC_tZ_LepFakes_Zjets() {

    // ========================
    // Configure top-level task
    // ========================
    AtlTopLevelAnalysis *main = new AtlTopLevelAnalysis("AtlSgTopFCNC_tZ_LepFakes_Zjets",
							"FCNC tZ, lepton fakes, Zjets");
    ...
    main->SetHistDir(Form("/rdsk/datm5/atlas/%s/sgtop_fcnc_tZ_lepfakes_zjets", user));
    main->SetJobHomeDir(Form("/rdsk/datm5/atlas/%s/sgtop_fcnc_tZ_lepfakes_zjets", user));
    main->SetTempDir(Form("/datc/atlas/%s/sgtop_fcnc_tZ_lepfakes_zjets.tmp", user));
    ...
    main->SetOutputTree("physics", "SgTopD3PD FCNC tZ lepton fakes Z+jets");
    ...
    main->SetToolParameter("BDecayGenTool", "fBJetMode", "AtlBDecayGenTool::kRandom");
