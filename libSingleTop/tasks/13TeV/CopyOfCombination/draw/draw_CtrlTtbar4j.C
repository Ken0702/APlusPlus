...
void draw_CtrlTtbar4j(const char* infile = "MCPlotter_CtrlTtbar4j.root",
                      Bool_t do_scale = kTRUE,
                      const char* channel = "",
                      Bool_t is_wjetsMerged = kFALSE) {
    //
    // Create control plots for the ttbar 4 jets control region
    //
    TFile *f = TFile::Open(infile);

    PlotHelper *plot = new PlotHelper();
    PlotHelper::fgRegion = new TString("#it{s}-chan. ttbar 4 jets VR, l+jets");
    TString cname = TString("CtrlTtbar4j_schan_lnu");
	...
	sfchoice = PlotHelper::kSchanTtbarCR_enu;
    } else if ( strcmp(channel, "_mu") == 0 ) {
	dir += channel;
	cname.Append(channel);
	sfchoice = PlotHelper::kSchanTtbarCR_munu;
    } else {
	sfchoice = PlotHelper::kSchanTtbarCR_lnu;
    }
    dir += "/Sel";
