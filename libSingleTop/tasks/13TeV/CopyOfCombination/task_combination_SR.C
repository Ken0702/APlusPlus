{
    ...
    // cut options
    Bool_t OnlyOdd = kTRUE ; 			//select only odd evennumber events
    ...
    const char* version = Form("v02_v34lj_OnlyOdd_%s", MC_v_str[mc_v]);
    /////////////
...
/////VERSION 32
   ...
   // const char* version = Form("TEST_%s", MC_v_str[mc_v]);
    ...
    const char* anadir = Form("/rdsk/datm3/atlas/%s/combination/A++/%s/%s", user, version, ext);
