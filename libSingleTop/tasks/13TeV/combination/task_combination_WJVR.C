{
    ...
    Bool_t UseDefaultCuts = kFALSE; //
    Bool_t UseTtbar3JVRCuts = kFALSE;
    Bool_t UseWjetsVRCuts = kTRUE;
    ...
    //const char* version = Form("v02_v34lj_WjetsVR_QCD_%s", MC_v_str[mc_v]);
    const char* version = Form("v02_v34lj_WjetsVR_SampleSyst_%s", MC_v_str[mc_v]);
  //const char* version = Form("v02_v34lj_WjetsVR_NOMTW_%s", MC_v_str[mc_v]);
    //const char* version = Form("v02_v34lj_WjetsVR_NOMET_%s", MC_v_str[mc_v]);
    //const char* version = Form("v02_v34lj_WjetsVR_NOMTW_QCD_%s", MC_v_str[mc_v]);
    //const char* version = Form("v02_v34lj_WjetsVR_NOMET_QCD_%s", MC_v_str[mc_v]);
    /////////////
...
//    const char* GridIdSuffix = "_v25_d_test";   // some control for grid job names
   ...
   if(UseTtbar3JVRCuts){
    ...
    main->SetPriority(10); // >10 means only submit to as2xb 
    main->SetDebugBuild();
    // main->SetDebug();

    //main->SetNProcessNthEventsOnly(10000); //Process every nth event
