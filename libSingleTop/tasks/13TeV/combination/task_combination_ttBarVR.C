{
    ...
    const char* version = Form("v02_v34lj_Ttbar3jVR_SampleSyst_%s", MC_v_str[mc_v]);
...
/////VERSION 32
   ...
   // const char* version = Form("TEST_%s", MC_v_str[mc_v]);
	...
	const char* anadir = Form("/rdsk/dat18/atlas/%s/combination/A++/%s/%s", user, version, ext);
...
//    const char* GridIdSuffix = "_v25_d_test";   // some control for grid job names
   ...
   if(UseTtbar3JVRCuts){
    ...
    //main->SetPriority(45); // >10 means only submit to as2xb, >40 as1xb 
    //main->SetDebugBuild();
    // main->SetDebug();
