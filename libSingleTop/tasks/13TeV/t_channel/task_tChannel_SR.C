...
//    const char* plotter_scheme = "SgTop_s_splitall";
   ...
   // const char* version = Form("v22_%s_NoMETCutForQCDFit_%s",sgtop_v, MC_v_str[mc_v]);
    const char* version = Form("v24_v29lj_RandomTest_%s", MC_v_str[mc_v]);
    ...
    //                           user, version, ext);
    const char* anadir = Form("/rdsk/datm7/atlas/%s/13TeV_t_channel/A++/%s/%s",
                              user, version, ext);
...
//    const char* GridIdSuffix = "_v25_d_test";   // some control for grid job names
    ...
    gROOT->Macro("task_tChannel_main.C");
    // main->SetDebug(1);
