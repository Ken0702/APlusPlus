...
void draw_CtrlWjets(const char* infile = "MCPlotter_CtrlWjets.root",
		    Bool_t do_scale = kTRUE,
		    const char* channel = "",
		    Bool_t is_wjetsMerged = kFALSE) {
    //
    // Create control plots for the W+jets control region
    //
    TFile *f = TFile::Open(infile);
    
    PlotHelper *plot = new PlotHelper();
    PlotHelper::fgRegion = new TString("#it{s}-chan. W+jets VR, l+jets");
    TString cname = TString("CtrlWjets_schan_lnu");
	...
	sfchoice = PlotHelper::kSchanWCR_enu;
    } else if ( strcmp(channel, "_mu") == 0 ) {
	dir += channel;
	cname.Append(channel);
	sfchoice = PlotHelper::kSchanWCR_munu;
    } else {
	sfchoice = PlotHelper::kSchanWCR_lnu;
    }
    dir += "/Sel";
