...
void task_fit_wjets(const char* indir = ".",
...
//    char const* version = "v04_Max_files_on_lep_withpulls_2_rebin";//"v03_QCDtests_reco_fake_param";
//    TString InputDir = Form("/rdsk/datm5/atlas/%s/13TeV_s_channel/%s/sgtop_schan_wjets", user, version);    
    // TString InputDir = "/rdsk/datm5/atlas/gottwalm/13TeV_s_channel/v04_QCDtests_Nedaa_eff_files_trigsplit/sgtop_schan_wjets/param_split_on_tag77";
//    TString InputDir = "/rdsk/datm5/atlas/gottwalm/13TeV_s_channel/v04_QCDtests_Nedaa_eff_files_trigsplit/sgtop_schan_wjets/param_split_on_tag77_2";
    TString InputDir = indir;
    TString OutputDir = Form("/rdsk/datm4/atlas/%s/HistFactory/13TeV_s_channel/%s/sgtop_schan_wjets", user, version);

    main->SetJobHomeDir(Form("%s/JobHome", OutputDir.Data()));
    main->SetTempDir(Form("/datc/atlas/%s/sgtop_schan_wjets.tmp", user));
    const char* path_el = Form("%s/%s/2Jets/enu/", InputDir.Data(), main->GetName());
    const char* path_mu = Form("%s/%s/2Jets/munu/", InputDir.Data(), main->GetName());
    const char* path_l = Form("%s/%s/2Jets/lnu/", InputDir.Data(), main->GetName());
...
//    main->SetInputDir(Form("%s/%s/input", main->GetHistDir()->Data(), main->GetName()));
    ...
    AtlHistFactoryMeasurement *meas = new AtlHistFactoryMeasurement("s_channel_analysis_wjets",
								    "s-channel analysis fit, W+jets region",
								    OutputDir.Data());
...
//    ch1->SetRebinDiscriminant(2);
    ...
    case 2:
        ...
        break;
    default: 
        Error(__FUNCTION__, "Wrong channel; chose \"lep\", \"mu\" or \"el\"");
        gSystem->Abort(0);
    }
