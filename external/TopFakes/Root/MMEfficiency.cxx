...
//     std::cout << "  MMEfficiency::INFO: Dummy Hstogram added." << std::endl;
  ...
  // if(fAverage<=0){
    ...
    if(!isDummy) SetAverageFromHist((TH1F*)fHist[fNvar]->Clone());
      ...
      std::cout << "  MMEfficiency::AddHisto: fAverage="<< fAverage << ", fNvar=" << fNvar << std::endl;
  }
  fDim[fNvar] = 1;
  fNvar += 1;
  if(isDummy) delete h;
...
void MMEfficiency::SetAverageHist(std::string fileName, std::string histoName){
  ...
  if(!h){
    delete f;
    std::cout << "  MMEfficiency::SetAverageHist: Histogram not found, not setting average efficiency" << std::endl;
    return;
  }
  fAverage = h->GetBinContent(1);
  std::cout << "  MMEfficiency::SetAverageHist: Setting Average Efficiency = " << fAverage << std::endl;
  delete f;
}
...
// cout << fHist[i]->GetTitle() << endl;
      if ( gDebug > 0 ) {
          // Info(__FUNCTION__, "Var #%d of histogram \"%s\" in directory %s: "
          //      "x-value (uncorrected for range] %f (%f), eff %f",
          //      i, fHist[i]->GetTitle(), fHist[i]->GetDirectory()->GetTitle(),
          //      v0, fVars[i], eff1);
          std::cout << "<MMEfficiency::GetEfficiency()>: Var #" << i
                    << ", Histogram \"" << fHist[i]->GetTitle()
//                    << "\", Directory \"" << fHist[i]->GetDirectory()->GetName()
                    << "\", x-value " << fVars[i]
                    << ", x in corrected xrange " << v0
                    << ", eff " << eff1 << std::endl;
      }
    }
    //
