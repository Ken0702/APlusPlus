...
:     m_runEigenVectorMethod(false), m_absEtaStrategy(GiveUp), m_otherStrategy(Flag)
{
  ...
  if (calibrationBName.size() > 0) {
    do {
      ...
      calibrationBNames.push_back(calibrationBName.substr(0,end));
  ...
  if (calibrationCName.size() > 0) {
    do {
      ...
      calibrationCNames.push_back(calibrationCName.substr(0,end));
  ...
  if (calibrationTName.size() > 0) {
    do {
      ...
      calibrationTNames.push_back(calibrationTName.substr(0,end));
  ...
  if (calibrationLightName.size() > 0) {
    do {
      ...
      calibrationLightNames.push_back(calibrationLightName.substr(0,end));
  ...
  TString strategy = env.GetValue("OutOfBoundsEta", "GiveUp");
  ...
  strategy = env.GetValue("OutOfBoundsOther", "Flag");
							...
							unsigned int mapIndex) const
{
  ...
  return getScaleFactor(variables, indexSF, indexEff, unc, numVariation);
}
							...
							Uncertainty unc, unsigned int numVariation) const
{
  ...
  if (unc == SFEigen || unc == SFNamed) {
    ...
    double valueUp;
    double valueDown;
    CalibrationDataContainer::CalibrationStatus statUp   = container->getResult(variables, valueUp,  up,   extrapolate);
    CalibrationDataContainer::CalibrationStatus statDown = container->getResult(variables, valueDown,down, extrapolate);
    if (statUp == CalibrationDataContainer::kError || statDown == CalibrationDataContainer::kError)
      return Analysis::dummyResult;
    if (m_otherStrategy == GiveUp)
      assert (statUp != CalibrationDataContainer::kRange); // no need to test also statDown
    else if (m_otherStrategy == GiveUpExtrapolated && extrapolate)
      assert (statUp != CalibrationDataContainer::kRange); // no need to test also statDown
    else if (m_otherStrategy == Flag) 
      if (statUp == CalibrationDataContainer::kRange)
	const_cast<CalibrationDataInterfaceROOT*>(this)->increaseCounter(indexSF, (extrapolate ? Extrapolated : Main));

    // return std::make_pair<double, double>(MCMCSF*valueUp, MCMCSF*valueDown);
    Analysis::CalibResult result(MCMCSF*valueUp, MCMCSF*valueDown);
    return result;
  ...
  double value;
  CalibrationDataContainer::CalibrationStatus status = container->getResult(variables, value);
  if (status == CalibrationDataContainer::kError)
    return Analysis::dummyResult;
  if (m_otherStrategy == GiveUp)
    assert (status != CalibrationDataContainer::kRange); // no need to test also statDown
  else if (m_otherStrategy == Flag)
    if (status == CalibrationDataContainer::kRange)
  ...
  if (unc == Total || unc == Systematic) {
    ...
    if (container->getUncertainty("extrapolation", variables, resSyst) == CalibrationDataContainer::kError)
  ...
  // Analysis::CalibResult result = std::make_pair<double, double>(MCMCSF*value, MCMCSF*uncertainty);
  Analysis::CalibResult result(MCMCSF*value, MCMCSF*uncertainty);
							 ...
							 unsigned int index, Uncertainty unc) const
{
  ...
  Analysis::CalibResult result(value, uncertainty);
						       ...
						       Uncertainty unc, unsigned int numVariation) const
{
  ...
  Analysis::CalibResult sfResult = getScaleFactor(variables, indexSF, indexEff, unc, numVariation);
							      ...
							      unsigned int numVariation, unsigned int mapIndex) const
{
  ...
  return getWeightScaleFactor(variables, indexSF, indexEff, unc, numVariation);
}
							      ...
							      Uncertainty unc, unsigned int numVariation) const
{
  ...
  if (! effContainer) return Analysis::dummyResult;
  ...
  double value;
  CalibrationDataContainer::CalibrationStatus status = container->getResult(variables, value);
  if (status == CalibrationDataContainer::kError) return Analysis::dummyResult;
  if (m_otherStrategy == GiveUp) assert (status != CalibrationDataContainer::kRange);
  else if (m_otherStrategy == Flag)
    if (status == CalibrationDataContainer::kRange) const_cast<CalibrationDataInterfaceROOT*>(this)->increaseCounter(indexSF);
  ...
  if (container->getUncertainty("MCreference", variables, refMCResult) == CalibrationDataContainer::kError)
    return Analysis::dummyResult;
  ...
  double fracMCnew;
  status = effContainer->getResult(variables, fracMCnew);
  if (status == CalibrationDataContainer::kError) return Analysis::dummyResult;
  if (m_otherStrategy == GiveUp) assert (status != CalibrationDataContainer::kRange);
  else if (m_otherStrategy == Flag)
    if (status == CalibrationDataContainer::kRange) const_cast<CalibrationDataInterfaceROOT*>(this)->increaseCounter(indexEff);
  ...
  if (unc == SFEigen || unc == SFNamed) {
    ...
    double valueUp;
    double valueDown;
    CalibrationDataContainer::CalibrationStatus statusUp   = container->getResult(variables, valueUp,  up,   extrapolate);
    CalibrationDataContainer::CalibrationStatus statusDown = container->getResult(variables, valueDown,down, extrapolate);
    if (statusUp == CalibrationDataContainer::kError || statusDown == CalibrationDataContainer::kError)
      return Analysis::dummyResult;
    if (extrapolate) {
      if (m_otherStrategy == GiveUpExtrapolated)
	assert (statusUp != CalibrationDataContainer::kRange);
      else if (m_otherStrategy == Flag)
	if (statusUp == CalibrationDataContainer::kRange)
    ...
    return std::make_pair(valueUp, valueDown);
  } //end eigenvector method
  ...
  if (unc == Total || unc == Systematic) {
    if (container->getSystUncertainty(variables, uncertaintyResult) == CalibrationDataContainer::kError)
