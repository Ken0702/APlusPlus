...
void showCalibration(TString fileName,
  ...
  if (c) {
	...
	if (ndim == 1) {
	  ...
	  cout << "\n-----------------------------------------------------------------" << endl;
	  for (Int_t bin = 0; bin < href->GetNbinsX(); ++bin) {
	    double uncstat = href->GetBinError(bin+1);
	    cout << " [" << setw(6) << href->GetBinLowEdge(bin+1) << ","  << setw(6) << href->GetBinLowEdge(bin+2) << "]"
		 << "  " << setw(8) << href->GetBinContent(bin+1) << "  " << setw(8) << uncstat;
	    if (hunc != href) {
	  ...
	  if (href->GetNbinsX() == 1) {
		...
		Int_t bin = href->GetBin(binx+1, biny+1);
